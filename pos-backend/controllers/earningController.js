...
const { startOfDay, endOfDay, subDays, startOfWeek, endOfWeek, subWeeks, startOfMonth, endOfMonth, subMonths, startOfYear, endOfYear, subYears, format } = require('date-fns');
...
const getZonedStartOfDayUtc = (date) => fromZonedTime(startOfDay(toZonedTime(date, TIMEZONE)), TIMEZONE);
const getZonedEndOfDayUtc = (date) => fromZonedTime(endOfDay(toZonedTime(date, TIMEZONE)), TIMEZONE);
const getZonedStartOfWeekUtc = (date) => fromZonedTime(startOfWeek(toZonedTime(date, TIMEZONE), { weekStartsOn: 1 }), TIMEZONE); // Monday start
const getZonedEndOfWeekUtc = (date) => fromZonedTime(endOfWeek(toZonedTime(date, TIMEZONE), { weekStartsOn: 1 }), TIMEZONE);
const getZonedStartOfMonthUtc = (date) => fromZonedTime(startOfMonth(toZonedTime(date, TIMEZONE)), TIMEZONE);
const getZonedEndOfMonthUtc = (date) => fromZonedTime(endOfMonth(toZonedTime(date, TIMEZONE)), TIMEZONE);
const getZonedStartOfYearUtc = (date) => fromZonedTime(startOfYear(toZonedTime(date, TIMEZONE)), TIMEZONE);
const getZonedEndOfYearUtc = (date) => fromZonedTime(endOfYear(toZonedTime(date, TIMEZONE)), TIMEZONE);
...
const calculateAndSaveDailyEarnings = async (req, res = null, next = null) => {
    try {
        const targetDate = req.body && req.body.date ? new Date(req.body.date) : subDays(new Date(), 1); // Default to yesterday
        ...
        const currentDayEarnings = todayOrderEarning ? todayOrderEarning.totalEarnings : 0;
        ...
        const previousDayEarningRecord = await DailyEarning.findOne({ date: startOfPreviousDay });
        const previousDayEarnings = previousDayEarningRecord ? previousDayEarningRecord.totalEarnings : 0;
        ...
        console.log(`[Earning Calculation] Saved daily earnings for ${formattedTargetDate}: ${currentDayEarnings} (Change: ${percentageChange}%)`);
        ...
        if (next) {
            ...
            res.status(500).json({ success: false, message: "Failed to calculate and save daily earnings." });
        }
...
const getDailyEarnings = async (req, res, next) => {
    try {
        ...
        const todayEarning = todayEarningRecord ? todayEarningRecord.totalEarnings : 0;
        const yesterdayEarning = yesterdayEarningRecord ? yesterdayEarningRecord.totalEarnings : 0;
        const percentageChange = todayEarningRecord ? todayEarningRecord.percentageChangeFromYesterday : calculatePercentageChange(todayEarning, yesterdayEarning);
...
const getPeriodEarnings = async (req, res, next) => {
    ...
    const numPeriods = parseInt(req.query.numPeriods) || (periodType === 'day' ? 7 : periodType === 'week' ? 4 : periodType === 'month' ? 12 : 5); // Default periods
    ...
    try {
        ...
        switch (periodType) {
            case 'day':
                ...
                groupByFormat = "%Y-%m-%d";
                addPeriodFunction = (date, i) => getZonedStartOfDayUtc(addDays(date, i));
                break;
            case 'week':
                startDate = getZonedStartOfWeekUtc(subWeeks(today, numPeriods - 1));
                groupByFormat = "%Y-%W"; // ISO week number
                addPeriodFunction = (date, i) => getZonedStartOfWeekUtc(addWeeks(date, i));
                break;
            case 'month':
                startDate = getZonedStartOfMonthUtc(subMonths(today, numPeriods - 1));
                groupByFormat = "%Y-%m";
                addPeriodFunction = (date, i) => getZonedStartOfMonthUtc(addMonths(date, i));
                break;
            case 'year':
                startDate = getZonedStartOfYearUtc(subYears(today, numPeriods - 1));
                groupByFormat = "%Y";
                addPeriodFunction = (date, i) => getZonedStartOfYearUtc(addYears(date, i));
                break;
            default:
                const error = createHttpError(400, "Invalid periodType. Use 'day', 'week', 'month', or 'year'.");
                return next(error);
        }
        ...
        for (let i = 0; i < numPeriods; i++) {
            ...
            else if (periodType === 'month') periodKey = format(periodDate, 'yyyy-MM');
            else if (periodType === 'year') periodKey = format(periodDate, 'yyyy');

            const foundRecord = earningsRecords.find(record =>
...
module.exports = { getDailyEarnings, getPeriodEarnings, calculateAndSaveDailyEarnings, getZonedStartOfDayUtc };